...
async function sendWithNodemailer(formData: any) {
  ...
  // @ts-ignore: Deno
  const receiverEmail = Deno.env.get('CONTACT_RECEIVER_EMAIL') || 'rsnirmiti@gmail.com';
  ...
  const mailOptions = {
    from: `"Nirmiti Contact Form" <${user}>`,
    to: receiverEmail,
    replyTo: formData.email,
    subject: `New Inquiry from ${formData.name}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        <h2 style="color: #c2410c; border-bottom: 2px solid #f3f4f6; padding-bottom: 10px;">New Website Inquiry</h2>
        ...
        <div style="background-color: #f9fafb; padding: 15px; border-radius: 4px; border-left: 4px solid #c2410c;">
          <strong>Message:</strong><br/>
          <p style="white-space: pre-wrap;">${formData.message}</p>
        </div>
        <p style="font-size: 12px; color: #6b7280; margin-top: 20px;">
          Sent from Nirmiti Associates Website
        </p>
      </div>
    `,
  };
